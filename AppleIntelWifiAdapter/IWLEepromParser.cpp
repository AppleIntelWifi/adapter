 ...
 *
 ******************************************************************************/
static void iwl_set_otp_access_absolute(IWLTransport *trans)
{
    trans->iwlRead32(CSR_OTP_GP_REG);
    
    trans->clearBit(CSR_OTP_GP_REG,
                    CSR_OTP_GP_REG_OTP_ACCESS_MODE);
}

static int iwl_nvm_is_otp(IWLTransport *trans)
{
    u32 otpgp;
    
    /* OTP only valid for CP/PP and after */
    switch (trans->m_pDevice->hw_rev & CSR_HW_REV_TYPE_MSK) {
        case CSR_HW_REV_TYPE_NONE:
            IWL_ERR(0, "Unknown hardware type\n");
            return -EIO;
        ...
        case CSR_HW_REV_TYPE_5150:
            return 0;
        default:
            otpgp = trans->iwlRead32(CSR_OTP_GP_REG);
...
static int iwl_init_otp_access(IWLTransport *trans)
{
    int ret;
    ...
    trans->iwlSetBitsPRPH(APMG_PS_CTRL_REG,
                          APMG_PS_CTRL_VAL_RESET_REQ);
    _udelay(5);
    trans->iwlClearBitsPRPH(APMG_PS_CTRL_REG,
                            APMG_PS_CTRL_VAL_RESET_REQ);
     ...
     */
    if (trans->m_pDevice->cfg->trans.base_params->shadow_ram_support)
...
static int iwl_read_otp_word(IWLTransport *trans, u16 addr,
                             __le16 *eeprom_data)
{
    int ret = 0;
    u32 r;
    ...
    trans->iwlWrite32(CSR_EEPROM_REG,
                      CSR_EEPROM_REG_MSK_ADDR & (addr << 1));
    ret = trans->iwlPollBit(CSR_EEPROM_REG,
                            CSR_EEPROM_REG_READ_VALID_MSK,
                            CSR_EEPROM_REG_READ_VALID_MSK,
                            IWL_EEPROM_ACCESS_TIMEOUT);
    if (ret < 0) {
        IWL_ERR(0, "Time out reading OTP[%d]\n", addr);
        return ret;
    }
    r = trans->iwlRead32(CSR_EEPROM_REG);
    /* check for ECC errors: */
    otpgp = trans->iwlRead32(CSR_OTP_GP_REG);
    if (otpgp & CSR_OTP_GP_REG_ECC_UNCORR_STATUS_MSK) {
        /* stop in this case */
        /* set the uncorrectable OTP ECC bit for acknowledgment */
        trans->setBit(CSR_OTP_GP_REG,
                      CSR_OTP_GP_REG_ECC_UNCORR_STATUS_MSK);
        IWL_ERR(0, "Uncorrectable OTP ECC error, abort OTP read\n");
        return -EINVAL;
    }
    if (otpgp & CSR_OTP_GP_REG_ECC_CORR_STATUS_MSK) {
        ...
        IWL_ERR(0, "Correctable OTP ECC error, continue read\n");
    }
 ...
 */
static bool iwl_is_otp_empty(IWLTransport *trans)
{
    ...
    if (!iwl_read_otp_word(trans, next_link_addr, &link_value)) {
        if (!link_value) {
            IWL_ERR(0, "OTP is empty\n");
            is_empty = true;
        }
    } else {
        IWL_ERR(0, "Unable to read first block of OTP list.\n");
        is_empty = true;
    }
 ...
 */
static int iwl_find_otp_image(IWLTransport *trans,
                              u16 *validblockaddr)
{
    ...
    do {
        ...
        IWL_INFO(0, "OTP blocks %d addr 0x%x\n",
        ...
        usedblocks++;
    } while (usedblocks <= trans->m_pDevice->cfg->trans.base_params->max_ll_items);
    
    /* OTP has no valid blocks */
    IWL_INFO(0, "OTP has no valid blocks\n");
    return -EINVAL;
}

static int iwl_eeprom_acquire_semaphore(IWLTransport *trans)
{
    u16 count;
    int ret = 0;
    ...
    for (count = 0; count < EEPROM_SEM_RETRY_LIMIT; count++) {
        ...
        /* See if we got it */
        ret = trans->iwlPollBit(CSR_HW_IF_CONFIG_REG,
                                CSR_HW_IF_CONFIG_REG_BIT_EEPROM_OWN_SEM,
                                CSR_HW_IF_CONFIG_REG_BIT_EEPROM_OWN_SEM,
                                EEPROM_SEM_TIMEOUT);
        if (ret >= 0) {
...
static void iwl_eeprom_release_semaphore(IWLTransport *trans)
{
    trans->clearBit(CSR_HW_IF_CONFIG_REG,
                    CSR_HW_IF_CONFIG_REG_BIT_EEPROM_OWN_SEM);
}

static int iwl_eeprom_verify_signature(IWLTransport *trans, bool nvm_is_otp)
{
    u32 gp = trans->iwlRead32(CSR_EEPROM_GP) & CSR_EEPROM_GP_VALID_MSK;
    
    IWL_INFO(0, "EEPROM signature=0x%08x\n", gp);
    
    switch (gp) {
        case CSR_EEPROM_GP_BAD_SIG_EEP_GOOD_SIG_OTP:
            if (!nvm_is_otp) {
                IWL_ERR(0, "EEPROM with bad signature: 0x%08x\n",
                        gp);
                return -ENOENT;
            }
            return 0;
        ...
        case CSR_EEPROM_GP_GOOD_SIG_EEP_MORE_THAN_4K:
            if (nvm_is_otp) {
                IWL_ERR(0, "OTP with bad signature: 0x%08x\n", gp);
                return -ENOENT;
            }
            return 0;
...
{
    __le16 *e;
    u32 gp = trans->iwlRead32(CSR_EEPROM_GP);
    int sz;
    int ret;
    u16 addr;
    ...
    sz = trans->m_pDevice->cfg->trans.base_params->eeprom_size;
    IWL_INFO(0, "NVM size = %d\n", sz);
    ...
    if (ret < 0) {
        IWL_ERR(0, "EEPROM not found, EEPROM_GP=0x%08x\n", gp);
        goto err_free;
    }
    ...
    if (ret < 0) {
        IWL_ERR(0, "Failed to acquire EEPROM semaphore.\n");
        goto err_free;
    }
    ...
    if (nvm_is_otp) {
        ...
        if (ret) {
            IWL_ERR(0, "Failed to initialize OTP access.\n");
            goto err_unlock;
        }
        
        trans->iwlWrite32(CSR_EEPROM_GP,
                          trans->iwlRead32(CSR_EEPROM_GP) &
...
static void iwl_eeprom_enhanced_txpower(IWLDevice *dev,
                    struct iwl_nvm_data *data,
                    ...
                    int n_channels)
{
    ...
    for (idx = 0; idx < entries; idx++) {
        ...
        IWL_INFO(0, "%s %d:\t %s%s%s%s%s%s%s%s (0x%02x)\n",
...
static void iwl_mod_ht40_chan_info(IWLDevice *dev,
                ...
                const u8 *eeprom, size_t eeprom_size)
{
    ...
    for (band = 1; band <= 5; band++) {
        ...
        for (ch_idx = 0; ch_idx < eeprom_ch_count; ch_idx++) {
            ...
            int channel_band = (band == 1) ? NL80211_BAND_2GHZ
                            : NL80211_BAND_5GHZ;
            int flags;
            if (channel_band == NL80211_BAND_2GHZ) {
            ...
            channel->ic_freq = ieee80211_ieee2mhz(channel->hw_value, flags);// ieee80211_channel_to_frequency(channel->hw_value, channel->band);

            /* set no-HT40, will enable as appropriate later */
            channel->ic_flags = IEEE80211_CHAN_PASSIVE;

//            if (!(eeprom_ch->flags & EEPROM_CHANNEL_IBSS))
//                channel->ic_flags |= IEEE80211_CHAN_NO_IR;
...
void iwl_init_ht_hw_capab(IWLTransport *trans,
              struct iwl_nvm_data *data,
...
//
//    if (!(data->sku_cap_11n_enable) ||
//        (trans->m_pDevice->iwlwifi_mod_params.disable_11n & IWL_DISABLE_HT_ALL) ||
//        !cfg->ht_params) {
...
static void iwl_init_sbands(IWLTransport *trans, const struct iwl_cfg *cfg,
                struct iwl_nvm_data *data,
                const u8 *eeprom, size_t eeprom_size)
{
              ...
              const u8 *eeprom, size_t eeprom_size)
{
    ...
    vm_size_t dt_size = sizeof(*data) + sizeof(ieee80211_channel) * IWL_NUM_CHANNELS;
    data = (struct iwl_nvm_data *)IOMalloc(dt_size);
    if (!data)
        return NULL;
    bzero(data, dt_size);
    ...
    if (!data->valid_tx_ant || !data->valid_rx_ant) {
        IWL_ERR_DEV(0, "invalid antennas (0x%x, 0x%x)\n",
                data->valid_tx_ant, data->valid_rx_ant);
        goto err_free;
    }
