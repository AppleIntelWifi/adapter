 ...
 */
static struct iwl_phy_db_entry *iwl_phy_db_get_section(
    struct iwl_phy_db *phy_db, enum iwl_phy_db_section_type type, u16 chg_id) {
...
int iwl_phy_db_set_section(struct iwl_phy_db *phy_db,
  ...
  enum iwl_phy_db_section_type type =
      (enum iwl_phy_db_section_type)le16_to_cpu(phy_db_notif->type);
  ...
  if (type == IWL_PHY_DB_CALIB_CHG_PAPD) {
    ...
    if (phy_db && !phy_db->calib_ch_group_papd) {
       ...
       */
      phy_db->calib_ch_group_papd = (struct iwl_phy_db_entry *)kcalloc(
    ...
    if (phy_db && !phy_db->calib_ch_group_txp) {
       ...
       * it to know how much we should allocate.
       */
      phy_db->calib_ch_group_txp = (struct iwl_phy_db_entry *)kcalloc(
          chg_id + 1, sizeof(struct iwl_phy_db_entry));
  ...
  IWL_INFO(0, "%s(%d): [PHYDB]SET: Type %d , Size: %d\n", __func__, __LINE__,
           type, size);
...
int iwl_phy_db_get_section_data(struct iwl_phy_db *phy_db, u32 type, u8 **data,
                                u16 *size, u16 ch_id) {
  struct iwl_phy_db_entry *entry;
  u16 ch_group_id = 0;
  ...
  entry = iwl_phy_db_get_section(phy_db, (enum iwl_phy_db_section_type)type,
                                 ch_group_id);
  if (!entry) return -EINVAL;
  ...
  *size = entry->size;
  ...
  IWL_INFO(0, "%s(%d): [PHYDB] GET: Type %d , Size: %d\n", __func__, __LINE__,
           type, *size);
...
static int iwl_send_phy_db_cmd(struct iwl_phy_db *phy_db, u16 type, u16 length,
  ...
  struct iwl_host_cmd cmd = {.id = PHY_DB_CMD};

  IWL_INFO(0, "Sending PHY-DB hcmd of type %d, of length %d\n", type, length);
...
static int iwl_phy_db_send_all_channel_groups(struct iwl_phy_db *phy_db,
  ...
  for (i = 0; i < max_ch_groups; i++) {
    ...
    if (err) {
      IWL_ERR(0, "Can't SEND phy_db section %d (%d), err %d\n", type, i, err);
      return err;
    }

    IWL_INFO(0, "Sent PHY_DB HCMD, type = %d num = %d\n", type, i);
  }
...
int iwl_send_phy_db_data(struct iwl_phy_db *phy_db) {
  ...
  IWL_INFO(0, "Sending phy db data and configuration to runtime image\n");
  ...
  if (err) {
    IWL_ERR(0, "Cannot send HCMD of  Phy DB cfg section\n");
    return err;
  }

  err = iwl_phy_db_get_section_data(phy_db, IWL_PHY_DB_CALIB_NCH, &data, &size,
                                    0);
  if (err) {
    IWL_ERR(0, "Cannot get Phy DB non specific channel section\n");
    return err;
  }

  err = iwl_send_phy_db_cmd(phy_db, IWL_PHY_DB_CALIB_NCH, size, data);
  if (err) {
    IWL_ERR(0, "Cannot send HCMD of Phy DB non specific channel section\n");
    return err;
  }

  /* Send all the TXP channel specific data */
  err = iwl_phy_db_send_all_channel_groups(phy_db, IWL_PHY_DB_CALIB_CHG_PAPD,
                                           phy_db->n_group_papd);
  if (err) {
    IWL_ERR(0, "Cannot send channel specific PAPD groups\n");
    return err;
  }

  /* Send all the TXP channel specific data */
  err = iwl_phy_db_send_all_channel_groups(phy_db, IWL_PHY_DB_CALIB_CHG_TXP,
                                           phy_db->n_group_txp);
  if (err) {
    IWL_ERR(0, "Cannot send channel specific TX power groups\n");
    return err;
  }

  IWL_INFO(0, "Finished sending phy db non channel data\n");
  return 0;
}
