...
class IWLMvmDriver : public OSObject {
 ...
 public:
  ...
  static void reqFWCallback(OSKextRequestTag requestTag, OSReturn result,
   ...
   */
  struct iwl_nvm_data *getNvm(IWLTransport *trans, const struct iwl_fw *fw);

  int loadUcodeWaitAlive(
      enum iwl_ucode_type ucode_type);  // iwl_mvm_load_ucode_wait_alive
  ...
  struct iwl_mcc_update_resp *updateMcc(
      const char *alpha2, enum iwl_mcc_source src_id);  // iwl_mvm_update_mcc

  int initMcc();  // iwl_mvm_init_mcc

  void rxChubUpdateMcc(
      struct iwl_rx_cmd_buffer *rxb);  // iwl_mvm_rx_chub_update_mcc
  ...
  int sendCmdPdu(u32 id, u32 flags, u16 len,
                 const void *data);  // iwl_mvm_send_cmd_pdu

  int sendCmdStatus(struct iwl_host_cmd *cmd,
                    u32 *status);  // iwl_mvm_send_cmd_status
  ...
  typedef int (*NewStateAction)(struct ieee80211com *, enum ieee80211_state,
                                int);
 ...
 public:
  ...
  static void rxFwErrorNotif(struct iwl_mvm *mvm,
