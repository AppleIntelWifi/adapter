...
static inline size_t iwl_mvm_chan_info_padding(IWLMvmDriver *mvm) {
  return iwl_mvm_has_ultra_hb_channel(mvm)
             ? 0
             : sizeof(struct iwl_fw_channel_info) -
                   sizeof(struct iwl_fw_channel_info_v1);
}
...
static inline void iwl_mvm_set_chan_info(IWLMvmDriver *mvm,
  ...
  if (iwl_mvm_has_ultra_hb_channel(mvm)) {
    ...
    struct iwl_fw_channel_info_v1 *ci_v1 = (struct iwl_fw_channel_info_v1 *)ci;
...
int iwl_phy_ctxt_add(IWLMvmDriver *drv, struct iwl_phy_ctx *ctxt,
  ...
  return iwl_phy_ctxt_apply(drv, ctxt, chains_static, chains_dynamic,
...
int iwl_phy_ctxt_apply(IWLMvmDriver *drv, struct iwl_phy_ctx *ctxt,
  ...
  if (iwl_mvm_has_ultra_hb_channel(drv)) IWL_INFO(0, "got ultra hb chan\n");

  u16 len = sizeof(cmd) - iwl_mvm_chan_info_padding(drv);
  iwl_phy_ctxt_cmd_hdr(drv, ctxt, &cmd, action, apply_time);
  iwl_phy_ctxt_cmd_data(drv, &cmd, ctxt->channel, chains_static,
                        chains_dynamic);

  ret = drv->sendCmdPdu(PHY_CONTEXT_CMD, 0, len, &cmd);
  if (ret) IWL_ERR(0, "Could not send phy context?\n");
...
void iwl_phy_ctxt_cmd_data(IWLMvmDriver *drv, struct iwl_phy_context_cmd *cmd,
  ...
  IWL_INFO(0,
           ...
           __func__, !!(chan->flags & APPLE80211_C_FLAG_2GHZ), chan->channel,
   ...
   */
  if (active_cnt == 1 &&
      (num_of_ant(iwl_mvm_get_valid_rx_ant(drv->m_pDevice)) != 1)) {
    idle_cnt = 2;
    active_cnt = 2;
  }

  iwl_phy_context_cmd_tail *tail =
      (iwl_phy_context_cmd_tail *)iwl_mvm_chan_info_cmd_tail(drv, &cmd->ci);
  ...
  tail->rxchain_info |= htole32(active_cnt << PHY_RX_CHAIN_MIMO_CNT_POS);
...
void iwl_phy_ctxt_cmd_hdr(IWLMvmDriver *drv, struct iwl_phy_ctx *ctxt,
  ...
  cmd->id_and_color = htole32(FW_CMD_ID_AND_COLOR(ctxt->id, ctxt->color));
